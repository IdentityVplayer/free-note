...
void main() {
  ...
  group('App update download directory', () {
    test('appDataDir points at the free_note data dir (not the repo)', () async {
      final dir = await StorageService.instance.appDataDir;
      expect(dir.path, endsWith('free_note'));
    });

    test('download folder lives under appDataDir and is cleared on launch',
        () async {
    ...
    test('clearDownloadFolder is safe when the folder does not exist',
        () async {
      final base = await StorageService.instance.appDataDir;
      final dl = Directory(p.join(base.path, 'download'));
