...
### Changed
- **更新下载目录改为系统下载目录** — 新版本 APK 现在保存到系统的「下载」目录（`/sdcard/Download`，Android）；其余平台回退到工作目录下的 `download/` 子目录。

### Added
- **首次启动引导** — 全新安装首次打开时显示引导页，讲解三步要点：① 选择笔记文件夹；② 长按文件夹按钮查看完整路径；③ 双击文件夹按钮重新选择/编辑。点「选择文件夹」后进入文件夹选择，点「稍后设置」可跳过（仅显示一次，状态持久化）。
- **文件夹按钮长按 / 双击手势** — 首页与设置页的文件夹（当前仓库）按钮支持**长按**弹出完整路径（可复制），**双击**重新选择文件夹；抽离为 `FolderChip` 组件统一复用。
- **私人笔记同步加固** — 同步排除规则统一为共享谓词 `isPathPrivate()`；拉取（pull）时不再复活远程残留的私密副本，同步成功提示区分公开/私密篇数。

## 1.18.0

### Added
- **文件名 = 标题联动** — 笔记内容文件名现在由标题派生（`sanitizeFileName(title).md`）；改标题时磁盘文件自动重命名（保留所在目录，同名自动加 ` (n)` 后缀，旧 `$id.md` 文件自动迁移）。
- **私人笔记文件夹** — 每个仓库打开/新建时自动创建 `私人笔记/` 目录；该目录及其内容**永不上传 GitHub**（同步上传与额外文件扫描均跳过，首页以锁图标标记）。
- **添加文件按钮** — 编辑器底部工具栏新增「添加文件」：用 `file_picker` 选择仓库内已有文件，自动填充相对路径并载入内容，标题取文件名。
- **剪切板小组件** — 新增首页第四底部标签「剪切板」：轮询系统剪贴板、去重保存历史（落盘 `clipboard.json`），点按条目复制回剪贴板、可单条删除或清空；设置页「小组件」可调历史条数上限（1–99999，默认 100）。
- **工具栏移到底部** — 编辑器 Markdown 格式工具栏与字数状态栏移至页面底部（`bottomNavigationBar`），标题与标签保持在顶部。

### Changed
- **存储路径规则统一** — `StorageService` 以「目录保留 + 基名跟随标题」为唯一派生规则，所有入口（编辑器、首页重命名/移动、新建）行为一致。
...
### Added
- **同步文件 ✓ 标识** — 成功同步后笔记卡片标题旁显示绿色 ✓。
- **同步进度条 + 日志** — GitHub 同步页面点击"立即同步"/"从 GitHub 拉取"后显示进度条、按钮锁定、实时日志，完成后恢复。
- **内置 APK 下载** — 检查更新时可直接下载 APK 到本地，无需跳转浏览器。
- **番茄钟卡片删除按钮** — 预设卡片上显示删除按钮（仅保留一个预设时隐藏）。
- **计时卡片背景跟随** — 当前计时器卡片显示激活预设的背景图片（高斯模糊）。

### Fixed
- **任务计划最后一个任务无法删除** — 移除 `_persist` 空列表守卫，该守卫会在删除最后一个任务时从磁盘回填列表导致删除被撤销。
- **自定义番茄钟无法删除** — 预设卡片增加删除入口（之前仅配置文件管理列表中有）。

## 1.14.0

### Added
- **长按文件/文件夹弹出操作菜单** — 首页笔记卡片和文件夹标题支持长按，弹出 BottomSheet 提供删除。
- **新建笔记自定义文件名和文件夹** — 点击添加笔记后弹出对话框输入文件名和选择所属文件夹，编辑器不再显示文件夹栏目。
- **笔记重命名/移动/复制** — 长按笔记可在 BottomSheet 中重命名、移动至其他文件夹、复制生成副本。

### Fixed
- **完成任务后页面不刷新** — `_persist` 空列表守卫改为从磁盘重载而非静默返回。
- **设置开关"子任务完成后自动完成主任务"不立即刷新** — 添加 `setState` 保证 UI 即时更新。
- **删除按钮文案错误** — 任务列表各处的删除按钮现在使用通用"删除"而非"删除笔记"。
- **Android 通知不提示** — 添加 `SCHEDULE_EXACT_ALARM` 权限，通知调度包裹 try-catch 避免静默失败。

### Changed
- **编辑器移除文件夹选择区域** — 文件夹选择移至新建笔记流程，编辑器不再显示保存位置/子文件夹行。
