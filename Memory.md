...
- `flutter_local_notifications: ^17.2.4`（沙箱缓存版，**无 Windows 支持**）
- `timezone: ^0.9.4`

### 已知缺口（务必告知用户）
- **Windows Toast 未完整实现**：`flutter_local_notifications` 17.2.4 缓存版不含 Windows 初始化（`WindowsInitializationSettings` 等未定义），NotificationService 已去掉 Windows 分支，仅 Android/iOS/macOS/Linux 完整。重复任务因 `respawnDueRepeats()` 在应用启动跑，**即使无 OS 投递也能工作**；但 Windows 端原生 Toast 提醒递延至后续版本。
